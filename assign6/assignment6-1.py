...
dummy2=np.zeros(dummy.shape)
dummy2=dummy1
M2,N2=dummy2.shape
#neighbour_size=(5,5)
#
#Neighbour_hood_mask=np.zeros(neighbour_size)
#M3,N3=Neighbour_hood_mask.shape
##Neighbour_hood_mask[0,0]=1
#for i in range(0,M2-M3+1):
#    for j in range(0,N2-N3+1):
#        if np.max(dummy2[i:i+M3,j:j+N3])>0:
#            dummy2[i:i+M3,j:j+N3]=dummy2[i:i+M3,j:j+N3]*Neighbour_hood_mask
#            dummy2[i,j]=255

dummy2=np.pad(dummy1,8,'constant')
for i in range(8,8+M2):
    for j in range(8,8+N2):
        if dummy2[i,j]==255:
            dummy2[i-8:i+8,j-8:j+8]=np.zeros((2*8,2*8))
            dummy2[i,j]=255            

plt.subplot(122)
plt.imshow(dummy2, cmap='gray', vmin =0, vmax =255 )
plt.show()
plt.title('After matched Fil image')
plt.xticks([])
plt.yticks([])
