# -*- coding: utf-8 -*-
"""
Created on Mon Dec  2 10:25:06 2019
...
def Guassian_MA(img, variance):
    ...
    for i in range(0, iterations):
        padded_image = np.zeros([3*M, 3*N])
        padded_image = Mirror_bound(img2)         
        Mov_Avg_output = Moving_avg_filt(padded_image, L, M, N)
...
def Guassian_pyramid(img1, variance, levels):
    ...
    for i in range(1, int(levels+1)):
        downsampled_image = Reduce(dummy1, variance)
        Guassian_list.append(downsampled_image)
        dummy1 = downsampled_image
...
def update(val):
    ...
    print(len(Guassian_pyramid_list),  len(Laplacian_pyramid_list))
